...
const char animalWords[20][4] = {"Duck", "Lion", "Bear", "Lark", "Goat", "Fish", "Bird", "Deer", "Gull", "Worm",
                                 "Seal", "Wolf", "Puma", "Mule", "Dove", "Moth", "Wasp", "Fawn", "Toad", "Lamb"};
const char humanWords[20][4] = {"John", "Mary", "Kate", "Lexi", "Mark", "Luis", "Duke", "Paul", "Sara", "Dami",
                                "Lisa", "Rema", "Mike", "Lily", "Jake", "Tina", "Rose", "Juan", "Kate", "Jane"};
const char countryWords[20][4] = {"Demk", "Keny", "Germ", "Mali", "Togo", "Cand", "Aust", "Japn", "Norw", "Russ",
                                  "Peru", "Cuba", "Finl", "Cong", "Port", "Tanz", "Fiji", "Turk", "Nigr", "Belg"};
const char thingWords[20][4] = {"Book", "Belt", "Desk", "Lamp", "Fork", "Silk", "Bell", "Coin", "Ring", "Bike",
                                "Sand", "Vest", "Bulb", "Ball", "Comb", "Dice", "Flag", "Milk", "Acid", "Soap"};
const char sportWords[20][4] = {"Golf", "Tenn", "Surf", "Race", "Dive", "Kick", "Jump", "Yoga", "Curl", "Ball",
                                "Ride", "Bowl", "Lift", "Judo", "Hike", "Disc", "Dart", "Hurl", "Puck", "Polo"};
...
// Generate a random element per category
void arrayIndex()
{
...
void checkIfWordGuessed()
{
  ...
  if (isWordGuessed == 0)
  {
    ...
    printf("Would you love to play again, kindly press button S1 to show the categories and then you guess new word!\n");
    isWordSelected = 0;
    isWordSet = 1;
    ...
    memset(wordArray, ' ', sizeof(wordArray));
    memset(rightOrderedLetters, 1, sizeof(rightOrderedLetters));
    wordIndex = 0;
    categoryCount = 0;
    endTimer();
    initTimer();
...
void setSecretWord(int categoryCount)
{
  ...
  else
  {
    ...
    puzzlePtr->category[10] = malloc(strlen("SPORT"));
    strcpy(puzzlePtr->category, "SPORT");

    for (int x = 0; x < 4; x++)
    {
      puzzlePtr->word[x][0] = sportWords[indexOfArray][x];
    }
  }
}

// Function to add a word to the wordArray
void appendLetterToWordArray(volatile char *letter)
{
...
ISR(PCINT1_vect)
{
  if (bit_is_clear(BUTTON_PIN, BUTTON1)) // Interrupt functionality of button 1
  {
    _delay_us(100);
  ...
  if (bit_is_clear(BUTTON_PIN, BUTTON2)) // Interrupt functionality of button 2
  {
    _delay_us(100);
    ...
    if (bit_is_clear(BUTTON_PIN, BUTTON2))
    {
      ...
      if (isWordSelected == 0 && isWordSet == 1) // Select the category
      {
        initTimer();
        puzzlePtr->time = 0;
        minutesElapsed = 0;
      ...
      {
        printf("\nPress button 1 to see more categories and Press the button 3 to confirm your choice\n");
      }
    }
  }

  if (bit_is_clear(BUTTON_PIN, BUTTON3)) // Interrupt functionality of button 3
  {
...
int main()
{
  ...
  while (isGamePlayed)
  {
    ...
    if (isWordSelected == 1 && isWordSet == 1)
    {
      ...
      {
        puzzlePtr->word[x][0];
        // printf(" %c",puzzlePtr->word[x][0]); // To see the guessed word on the serial monitor.
      }
    }
