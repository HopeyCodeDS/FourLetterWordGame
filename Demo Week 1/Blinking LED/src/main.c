...
int main() //Start of the program
{
    ...
    DDRB |= (1 << 4); //We could also use the << bitshift operator. DDRB = 0b00010000
    //DDRB |= (1 << (PB2 + 3)); //Here we use the port names from io.h to make the code more readable.

    while (1)
    {
        PORTB = (0 << (PB2 + 2)); //We write a 0 to PB2+3 to let the connected LED light up.
        _delay_ms(1000); //We use the _delay_ms(..) function from delay.h to pause the execution during 1000 milliseconds (1 sec).
        PORTB = (1 << (PB2 + 2)); //We write a 1 to PB2+3 to let the connected LED go dark.
        _delay_ms(1000);
    }
    return 0;
}